...
random.seed(12)
experimental = random.sample(sorted(experimental), math.floor(len(experimental)/2))
random.seed(33)
control = random.sample(sorted(control), math.floor(len(control)/2))



#participants = all_participants
participantsList = [experimental, control]
suffixList       = ['_2G_experimental', '_2G_control']
...
def update_ImageFolder(ImageFolder, cond_dict, df, targetCons, vps, imageDir, t1, t2):
	...
	df2 = df.loc[(df['condition'].isin(targetCons)) & (df['vp'].isin(vps)) & (df['trialnr'].between(t1, t2))]
